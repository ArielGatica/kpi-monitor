                        ...
                        <template v-slot:item.action="{ item }">
                            ...
                            <template v-slot:activator="{ on }">
                                <v-btn 
                                    ...
                                    <v-icon color="#FF3D00">
                                        {{ item.nombre == 'PIU Crash' ? 'play_circle_filled' : false || item.nombre == 'PIU Restart' ? 'play_circle_filled' : false }}
                                    </v-icon>
                                </v-btn>
                            </template>
            ...
            getDataMonitores().then((response) =>{
                if(response.status == 200){
                    ...
                    .then((responseCallbacks) =>{
                        ...
                        const groypByIdCallbacks = responseCallbacks.data.reduce((acc, data) =>{
                        ...
                        response.data.forEach((value) =>{
                            newSalidaMonitores.push({
                                ...
                                nombre: value.nombre,
                                estado: value.estado,
                                idCallbackDiagnostico: groypByIdCallbacks[value.idCallbackDiagnostico][0]['callback']
                            })
                        })
